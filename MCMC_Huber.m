...
parameters.max([1:3,38:40,43:45,52:54]) = 3;             %k_u input scale parameters
parameters.max([4:9, 46:51]) = 2.5;                      %KPF & KPINF. Neuronal interaction parameters
parameters.max([48 49]) = 3.5;  

parameters.min([10 11 12 13 14 15]) =log10(1/0.78);      %ksink for N and Ca2+
parameters.min(22) = 0;                                  %sinkNO
parameters.min([25 37]) = -12;                           %Km saturation parameters, allowed to be small

parameters.min([55 56 57]) =log10(1/0.78);               %ksink for N
...
parameters.max(41:42)=1;                                  % sign parameters
parameters.min(41:42)=-1;                                 % sign parameters


parameters.number = length(parameters.name);              % number of parameters

%% Create file where all chi-2 acceptable parameters are saved
FileName=sprintf('MCMC_Huber_%s.dat',datestr(now,'yymmdd_HHMMSS'));
FID = fopen(FileName,'wt');

%% Set the objective function
objectiveFunction=@(X) Cost_Huber(X,HuberData,Constants,stimend,FID);
...
%% set intital parameters and sigma structures
...
mkdir(pwd,['MCMC/MCMCHuber_' folderStr])
FolderName=fullfile(pwd,['MCMC/MCMCHuber_' folderStr]);
...
GenerateUncertaintyHuber(FolderName,FileName);
...
[~, Data, Constants, stimend, ~] = optsetupfunction(6);
...
parameters.max([4 5 6 7 8 9]) = 3;                       %KPF & KPINF
parameters.min([25 37]) = -12;                           %km is allowed to be small
parameters.min([10 11 12 13 14 15]) = log10(1/0.78);     %ksink 	    
parameters.max(1:3) = 2.5;  
parameters.min(22) = 0;                                  %sinkNO
 
% circuit parameters
parameters.min(30:32) = [0.01 log10(1.3) log10(1.4)];    %K123 circuit
parameters.min(33:35) = [1 1 1];                         %vis123 circuit
parameters.max(30:32) = [log10(2) log10(2) 8];           %K123 circuit
parameters.max(33:35) = [2 2.7 2.7];                     %vis123 circuit

% different set of vascular param for long stimulation
parameters.min(38:39) = [log10(1.3) log10(1.4)];
parameters.min(40:41) = [1 1];
parameters.max(38:39) = [log10(2) 8];
parameters.max(40:41) = [2.7 2.7];
...
FileName=sprintf('MCMC_DrewTimeVariant_%s.dat',datestr(now,'yymmdd_HHMMSS'));
FID = fopen(FileName,'wt');

%% set the objective function
objectiveFunction=@(X) Cost_Drew_TimeVariant(X,Data,Constants,stimend,FID);
...
mkdir(pwd,['MCMC/DrewTimeVariant_' folderStr])
FolderName=fullfile(pwd,['MCMC/MCMCDrewTimeVariant_' folderStr]);
...
GenerateUncertaintyDrewTimeVariant(FolderName,FileName);
